<template>
  <ImWrapper>
    ...
    <ImTableArea>
      <div class="mb-4">
        <el-button type="primary" @click="addItem">新建试卷</el-button>
      </div>
...
export default {
  ...
  methods: {
    viewItem(record) {
      this.$router.push({ name: '', query: { objectCode: record.objectCode } })
    },
    ...
    deleteItem(record) {
      this.$confirm('确定要删除该项吗？', '提示', {
        ...
        .then(() => {
          deleteItem({ objectCode: record.objectCode }).then(res => {
            if (res.code === 200) {
